...
import { Plus, Edit, Trash2, Search, Trash, Shield } from 'lucide-react';
...
function AdminListContent() {
    ...
    const fetchUsers = async () => {
        try {
            setLoading(true);
            const params: any = { page, limit, role: 'Admin,Super Admin' };
    ...
    const handleSubmit = async (data: any) => {
        try {
            ...
            if (selectedUser) {
                ...
                showSuccess('Data admin berhasil diperbarui');
            } else {
            ...
            showError(error.response?.data?.message || 'Gagal menyimpan data admin');
        } finally {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            showSuccess('Data admin berhasil dihapus');
            fetchUsers();
        } catch (error) {
            showError('Gagal menghapus data admin');
        }
    };
        ...
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-gray-600 mt-1">Kelola administrator dan super admin sistem</p>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <Card>
                    <CardBody>
                        <p className="text-sm text-gray-600">Total Keseluruhan Admin</p>
                    ...
                    <CardBody>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <p className="text-3xl font-bold text-purple-600">
                                {userList.filter(u => (u as any).role?.namaRole === 'Super Admin' || u.role === 'Super Admin').length}
                            </p>
                        </div>
                    </CardBody>
                </Card>
                    ...
                    <CardBody>
                        ...
                        <div className="flex items-center gap-2 mt-1">
                            ...
                            <p className="text-3xl font-bold text-blue-600">
                                {userList.filter(u => (u as any).role?.namaRole === 'Admin' || u.role === 'Admin').length}
                            </p>
                        </div>
                    </CardBody>
                </Card>
                ...
                <CardBody>
                    <div className="space-y-4">
                        ...
                        {loading ? (
                            ...
                            <div className="text-center py-8 text-gray-500">Tidak ada data admin</div>
                        ) : (
                            <>
                                    ...
                                    <table className="w-full">
                                        ...
                                        <tbody>
                                            {userList.map((user, index) => {
                                                const roleName = (user as any).role?.namaRole || user.role || 'Admin';
                                ...
                                <div className="flex items-center justify-between mt-4 py-3 border-t border-gray-100">
                                    <div className="text-sm text-gray-500">
                                        Menampilkan <span className="font-medium">{(page - 1) * limit + 1}</span> sampai <span className="font-medium">{Math.min(page * limit, totalItems)}</span> dari <span className="font-medium">{totalItems}</span> admin
                                    </div>
                                    <div className="flex items-center gap-2">
