...
const authorizeRoles = (...roles) => {
    return (req, res, next) => {
        // JWT payload sudah berisi role name (string) dari createTokenUser
        // Tidak perlu mapping lagi dari ID
        const userRoleName = req.user.role; // Langsung ambil dari payload

        console.log('🔐 Authorization Check:', {
            userRole: userRoleName,
            allowedRoles: roles,
            isAuthorized: roles.includes(userRoleName)
        });
